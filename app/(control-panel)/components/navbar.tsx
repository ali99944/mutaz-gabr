...
import { Home, Layout, Menu, MessageSquare, Newspaper, Palette, Settings, Users, X } from "lucide-react"
  ...
  return (
    <nav className="bg-white border-b sticky top-0 z-50">
          ...
          <div className="hidden md:flex items-center">
            <span className="text-gray-700 mr-4">{session?.user?.name}</span>
            {
              status == 'authenticated' && (
                <Button variant="ghost" className="mr-2 text-app-secondary hover:text-app-secondary/85" onClick={handleSignout}>
                  تسجيل الخروج
                </Button>
              )
            }
      ...
      <div className="max-w-7xl mx-auto bg-white px-4 py-2 min-h-14  flex items-center flex-wrap gap-2">
        ...
        <Button variant={'ghost'} className=" flex items-center text-primary">
          <Link href="/admin/settings">الاعدادات</Link>
